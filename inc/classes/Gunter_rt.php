<?php
/**
 * The Gunter_RT initiate the theme engine
 */
...
class Gunter_RT {
	...
	public function init_hooks() {
        ...
        if ( 'valid' != get_option( $this->theme_slug . '_purchase_code_status', false ) ) {
            ...
            if ( ( ! isset( $_GET['page'] ) || 'gunter' != $_GET['page'] ) ) {
	...
	function admin_error() {
		$out = '<div class="notice notice-error is-dismissible gunter-purchase-notice"><p>' . sprintf( wp_kses_post( __( 'The %s theme needs to be registered. %sRegister Now%s', 'gunter' ) ), 'Gunter', '<a href="' . admin_url( 'admin.php?page=gunter') . '">' , '</a>' ) . '</p></div>';
	...
	function admin_notice() {
		$out = '<div class="notice is-dismissible gunter-purchase-notice"><p>' .sprintf( wp_kses_post( __( 'Purchase key is invalid. Need a license? %sPurchase Now%s', 'gunter' ) ), '<a target="_blank" href="https://1.envato.market/Xav5M">', '</a>' ) .'</p></div>';
		if ( get_option('notice_dismissed') ) {
		    return;
        }
		echo wp_kses_post($out);
	...
	function messages() {
		...
		if ( $status != '' ) {
			...
			if($status == 'valid'){
				$title = esc_html__( 'Purchase Key Verified & Registered!', 'gunter' );
			}elseif($status == 'already_registered'){
				$title = esc_html__( 'Purchase Key Already Registered', 'gunter' );
			}else{
				$title = esc_html__( 'Purchase Key Invalid', 'gunter' );
			}
        } else {
            $license_icon = ($status == 'valid') ? '<i class="dashicons-yes"></i>' : '<i class="dashicons-warning"></i>';
		    $title = esc_html__( 'Verify Theme Purchase Key. . .', 'gunter' );
        }
	...
	function form() {
		...
		<form action="" method="post" id="verify-envato-purchase" class="et-theme-register-form">
			...
			<input id="gunter_purchase_code" name="gunter_purchase_code" type="text" value="<?php echo esc_attr( $license ); ?>" placeholder="<?php esc_attr_e( 'Enter purchase key', 'gunter' ); ?>" required>
			<?php if( $status != 'valid' ){ ?>
				<input type="submit" value="<?php esc_attr_e( 'Verify Now', 'gunter' ); ?>">
			<?php } ?>
			<?php
			if ( $status != '' ) {
				if( $status == 'valid' ){ ?>
					<input id="gunter_purchase_code" name="gunter_purchase_code" type="hidden" value="">
					<input type='submit' class='deactivate' value='Deregister Theme'>
					<?php
				}
			} ?>

			<?php if( get_option( 'gunter_purchase_code_status' ) == 'already_registered' ): ?>
				<div class="et_warning">
					<span class="dashicons dashicons-warning"></span>
					<?php echo stripslashes( get_option( 'gunter_already_registered' ) ); ?>
				</div>
			<?php endif; ?>

		</form>
		<?php
        if ( isset($_POST['gunter_purchase_code']) ) {
			if( $_POST['gunter_purchase_code'] != '' ){
				echo "<meta http-equiv='refresh' content='0'>";
				update_option( $this->theme_slug . '_purchase_code', $_POST['gunter_purchase_code'] );
				$purchase_code = htmlspecialchars($_POST['gunter_purchase_code']);
				...
				if ( is_object($o) && strpos($o->item_name, 'Gunter') !== false ) {
					...
					update_option( 'gunter_purchase_code_status', 'invalid', 'yes' );
				}
			}else{
				echo "<meta http-equiv='refresh' content='0'>";
				
				$purchase_code = get_option( 'gunter_purchase_valid_code' );
