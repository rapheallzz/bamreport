...
<?php if( $enable_blog_banner == true ): ?>
	<div class="page-title-area uk-page-title" <?php if( !$bg_image == '' ){?>style="background-image: url(<?php echo esc_url($bg_image );?>);"<?php } ?>>
		<div class="uk-container">
			<h1> <?php printf( esc_html( 'Search Results for : %s', 'gunter' ), '<span>' . get_search_query() . '</span>' ); ?> </h1>
			...
			if ( function_exists('yoast_breadcrumb') ) {
				...
				<ul>
					...
					<li><?php if(!$title == ''){ echo esc_html($title); }else{ echo esc_html_e('Search', 'gunter'); } ?> </li>
				</ul>
			<?php } ?>
		</div>
	</div>
				...
				<div class="custom-row">
					...
					<?php if(!$pagin == '') { ?>
						<div class="pagination-area">
							...
							<?php echo wp_kses_post($pagin, 'gunter'); ?>
							</nav>
						</div>
					<?php } ?>
				</div>
